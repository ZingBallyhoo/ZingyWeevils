        ...
        public async Task<NestConfig> GetNestConfig([FromBody] GetNestConfigRequest request)
        {
            ...
            var dto = await m_dbContext.m_weevilDBs
                ...
                .Select(weev => new
                {
                    ...
                    m_items = weev.m_nest.m_items.Where(item => item.m_placedItem != null).Select(y => new NestItem
                    {
                        m_databaseID = y.m_id,
                        m_color = y.m_color,
                        m_configName = y.m_itemType.m_configLocation,
                        m_category = (int)y.m_itemType.m_category,
                        m_powerConsumption = y.m_itemType.m_powerConsumption,
                        m_locID = y.m_placedItem!.m_roomID,
                        m_currentPos = y.m_placedItem!.m_posAnimationFrame,
                        m_placedOnFurnitureID = y.m_placedItem!.m_placedOnFurnitureID ?? 0,
                        m_spot = y.m_placedItem!.m_spotOnFurniture,
                    }),
                    m_gardenItems = weev.m_nest.m_gardenItems.Where(x => x.m_placedItem != null).Select(y => new GardenItem
                    {
                        m_databaseID = y.m_id,
                        m_color = y.m_color,
                        m_fileName = y.m_itemType.m_configLocation,
                        m_category = (int)y.m_itemType.m_category,
                        m_powerConsumption = y.m_itemType.m_powerConsumption,
                        m_r = y.m_itemType.m_boundRadius,
                        m_locID = y.m_placedItem!.m_roomID,
                        m_x = y.m_placedItem!.m_x,
                        m_z = y.m_placedItem!.m_z,
                    }),
                    m_score = 
            ...
            nestConfig.m_items.AddRange(dto.m_items);
            nestConfig.m_items.AddRange(dto.m_gardenItems);
